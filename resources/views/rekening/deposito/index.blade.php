...
@section('title', 'Rekening Deposito | '.config('app.name'))
...
@section('content')
    ...
    <div class="bg-white shadow rounded-xl border-l-4 border-[#F36F21] p-6">
        <div id="loading" class="flex items-center gap-2 text-gray-500 text-sm italic">
            ...
            <span>Memuat data deposito...</span>
        </div>
        <div id="tableContainer" class="hidden overflow-x-auto">
            <table class="min-w-full text-sm border-collapse">
                ...
                <tbody id="depositoTable" class="divide-y divide-gray-100">
                </tbody>
            </table>
        </div>
    </div>
...
@push('scripts')
    <script>
        (async function () {
            ...
            try {
                const {data} = await axios.get('/secure/portofolio-deposito');
                if (data?.responseCode !== '00') notify('error', 'Gagal memuat data deposito')
                ...
                if (!list.length) {
                    ...
                    <span>Tidak ada rekening deposito aktif.</span>`;
                    return;
                }
 ...
 <td class="px-4 py-2 text-center">
                   <a href="/rekening/deposito/detail?norek=${encodeURIComponent(t.noRekening || t.no_rekening)}"
   class="inline-flex items-center gap-1 text-[#F36F21] hover:text-[#003D73] text-sm font-medium transition">
    ...
    Detail
        ...
        `).join('');
                ...
                console.error('Error deposito:', err);
